...
const client = new Client
    ...
    if (message.customId == "open-ticket") {
      if (client.guilds.cache.get(message.guildId).channels.cache.find(c => c.topic == message.user.id)) {
      ...
      message.guild.channels.create(`ticket-${message.user.username}`, {
        ...
        const embed = new client.discord.MessageEmbed()
          .setColor('6d6ee8')
          .setAuthor({name: `${message.user.username}'s Ticket`, iconURL: 'https://i.imgur.com/oO5ZSRK.png'})
          .setDescription('Select the category of your ticket')
          .setFooter({text: `${message.user.tag} || github.com/blackknight683`, iconURL: client.user.displayAvatarURL()})
          .setTimestamp();
            ...
            if (msg.deletable) {
              msg.delete().then(async () => {
                const embed = new client.discord.MessageEmbed()
                  ...
                  .setDescription(`<@!${message.user.id}> Created a ticket with issues regarding \`${i.values[0]}\``)
